...
# ============================================================
    ...
    st.info(
        "Upload CSV datasets and write multiple SQL statements in a single "
        "editor. Select the statement(s) you want to execute."
    )
    ...
    if files:
        ...
        if tables:
            ...
            st.caption(
                "You can write multiple SQL statements in this single box. "
                "Separate statements with a semicolon (;)."
            )

            # One stable editor for all SQL code.
            sql_code = st.text_area(
                ...
                height=300,
                key="single_sql_editor"
            )

            st.session_state.single_sql_code = sql_code

            # Split the editor into executable statements.
            # A semicolon marks the end of a statement.
            statements = [
                s.strip()
                for s in sql_code.split(";")
                if s.strip()
            ]

            # Remove leading SQL comments when checking whether a statement
            # actually contains executable SQL.
            ...
            for statement in statements:
                ...
                if cleaned:
                    executable_statements.append(
                        {
                            "number": len(executable_statements) + 1,
                            "code": cleaned
                        }
                    )
            ...
            if executable_statements:
                ...
                st.subheader("3. Select SQL Statement(s)")

                statement_labels = [
                    f"Statement {s['number']}: "
                    f"{s['code'].splitlines()[0][:80]}"
                    for s in executable_statements
                ]

                selected_statements = st.multiselect(
                    "Select the SQL statement(s) to execute",
                    options=statement_labels,
                    default=statement_labels[:1],
                    key="selected_sql_statements"
                )

                # Show exactly what the participant selected.
                if selected_statements:

                    st.markdown("**Selected code:**")

                    for label in selected_statements:

                        selected_index = statement_labels.index(label)

                        st.code(
                            executable_statements[selected_index]["code"],
                            language="sql"
                        )
                ...
                with e1:
                    ...
                    run_selected = st.button(
                        "▶ Run Selected Statement(s)",
                        type="primary",
                        use_container_width=True,
                        key="run_selected_sql_statements"
                    )
                ...
                with e2:
                    ...
                    run_all = st.button(
                        "▶ Run All Statements",
                        use_container_width=True,
                        key="run_all_sql_statements"
                    )

                def execute_sql_statements(indices):
                    ...
                    try:
                        ...
                        for statement_index in indices:

                            query = executable_statements[
                                statement_index
                            ]["code"]

                            try:

                                # SELECT / WITH / PRAGMA queries return
                                # a DataFrame.
                ...
                if run_selected:
                    ...
                    if not selected_statements:
                        ...
                        st.warning(
                            "Please select at least one SQL statement."
                        )

                    else:

                        indices = [
                            statement_labels.index(label)
                            for label in selected_statements
                        ]

                        outputs = execute_sql_statements(indices)

                        st.session_state.single_sql_outputs = outputs
                ...
                if run_all:
                    ...
                    indices = list(
                        range(len(executable_statements))
                    )
                ...
                # Results are displayed under the corresponding statement.
                ...
                if outputs:
                    ...
                    for statement_index, output in outputs.items():

                        st.markdown(
                            f"### Statement {statement_index + 1}"
                        )
                ...
                st.info(
                    "Enter SQL statements above. Separate multiple "
                    "statements with ;"
                )
