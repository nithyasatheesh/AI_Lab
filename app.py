...
# ============================================================
    ...
    st.info(
        "You can start by typing Python code directly. "
        "Upload data or a Jupyter Notebook only when required."
    )
    ...
    st.subheader("A. Write Python Code")
    ...
    if data_files:
        ...
        if dataframes:
            ...
            st.info(
                "The first uploaded dataset is available as `df`."
            )
    ...
    # --------------------------------------------------------
    # RUN CODE WITH UPLOADED DATA
    # --------------------------------------------------------

    if st.session_state.uploaded_dataframes:

        st.subheader(
            "C. Run Python Code Using Uploaded Data"
        )
    ...
    st.subheader("D. Upload Jupyter Notebook")

    st.info(
        "Upload a `.ipynb` file to load its Python code cells. "
        "You can edit, run, delete and add cells."
    )
    ...
    if notebook_file:
        ...
        try:
            ...
            if (
                st.session_state.notebook_loaded_name
                != file_id
            ):
                ...
                # Keep uploaded data available to notebook
            ...
            if not cells:
                ...
                # Individual cells
                for i in range(
                    len(st.session_state.notebook_cells)
                ):
                    ...
                    with c2:
                        ...
                        if st.button(
                            ...
                            st.session_state.notebook_cells.pop(
                                i
                            )
